...
def make_token(*, lifetime=timedelta(minutes=10)):
    return CapabilityToken.issue(
        ...
        allowed_paths=("agents/checkpoints/",), lifetime=lifetime, now=NOW,
    )
...
def test_expired_token_is_rejected():
    ...
    with pytest.raises(CapabilityError, match="expired"):
        CapabilityToken.verify(token.to_dict(), signing_key=b"host-secret", now=NOW + timedelta(seconds=1))
...
def test_supervisor_expiry_revokes_kills_and_wipes_once():
    ...
    supervisor = WorkerSupervisor(
        token, signing_key=b"host-secret", kill_vm=lambda: calls.append("kill"),
        wipe_vm=lambda: calls.append("wipe"), revoke_token=lambda _: calls.append("revoke"),
...
def test_destructive_host_operation_trips_boundary():
    calls: list[str] = []
    supervisor = WorkerSupervisor(
        make_token(), signing_key=b"host-secret", kill_vm=lambda: calls.append("kill"),
        wipe_vm=lambda: calls.append("wipe"), revoke_token=lambda _: calls.append("revoke"), now=lambda: NOW,
    )
