...
- **`docs/POSITIONING.md`** (new): positioning vs the class of restrictive enterprise agent platforms (Microsoft Copilot Studio lead example, plus Vertex AI Agent Builder, Bedrock Agents, Agentforce, ServiceNow Now Assist, watsonx Orchestrate, Rovo, Glean). Connector gap, friction comparison, security comparison, close-the-gap plan (rejected expansions: no-code authoring, marketplace, centralised tenant admin; committed work: M365 + Slack connectors, MSAL auth, per-resource gating, compliance control mapping). Decision matrix.
- **README**: added "Aim: less friction than Copilot Studio, no less secure" section after "What Testudo is and is not". Generalised the not-a-replacement bullet to cover the full-fledged-orchestrator category (Hillstar, Airflow, Prefect, Dagster, Temporal, Argo Workflows). Reframed the no-code bullet around agentic-tooling failure modes rather than user populations; explicit low-code-yes-no-code-no position.
- **ROADMAP**: new v0.1.7 milestone (Microsoft 365 + Slack connectors + MSAL auth + per-resource gating + compliance scaffolds). Behind v0.1.6 (Docker) in sequence.
- **NEXT_ACTIONS**: Priority 0.5 with v0.1.7 release-blockers.
- **Mypy strict mode green**: 21 errors fixed (Decision Literal annotations across `sanitisers/*`; typed `result` / `response` variables in `mcp_servers/base.py`; lambda -> def in `prompts/template.py`; typed local in `models/ollama.py`; `Any` annotation in `server/rate_limit.py`). Per-module overrides for optional-extra modules (`pypdf`, `docx`, `databricks`, `yaml`) added to `[tool.mypy]` in pyproject.toml.
- **`.pre-commit-config.yaml`**: ruff-format + ruff (--fix) + mypy + trailing-whitespace + end-of-file-fixer + check-yaml / toml / json + detect-private-key + large-file checks. Mirrors the GitHub Actions CI sequence locally; runs on every commit.
- **Repo on GitHub**: public at `github.com/evoclock/testudo` after force-push of local history (the auto-generated `Initial commit` was just a stub LICENSE). History scrubbed of a previously-leaked Databricks PAT via `git-filter-repo --replace-text`; token was rotated in the Databricks console first. CI green on `test(3.11)` and `test(3.12)` matrix jobs.
